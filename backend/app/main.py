...
def get_suggested_question(original: str, answer: str, results: List[dict]) -> Optional[str]:
    ...
    for r in results:
        ...
        similarity = text_similarity(original, question)
        if similarity > 0.6:
            continue
        
        term_matches = sum(1 for w in answer_words if w in question.lower())
        candidates.append({"question": question, "similarity": similarity, "term_matches": term_matches})
...
async def lifespan(app: FastAPI):
    global retriever
    print("Starting Quantum Computing LLM API (Groq)...")
    validate_config()
    retriever = Retriever()
    print("Ready")
    yield
